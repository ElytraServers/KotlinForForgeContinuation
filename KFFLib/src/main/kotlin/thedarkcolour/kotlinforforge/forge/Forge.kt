 ...
 * An alternative to using [net.minecraftforge.fml.DistExecutor]
 */
public inline fun <T> runForDist(
    clientTarget: () -> T,
    serverTarget: () -> T,
): T =
    if (DIST == Dist.CLIENT) {
...
public inline fun registerConfig(
    type: ModConfig.Type,
 ...
 */
public fun <T> lazySidedDelegate(
    clientValue: () -> T,
    serverValue: () -> T,
): ReadOnlyProperty<Any?, T> = LazySidedDelegate(clientValue, serverValue)
 ...
 * @param T the common type of both values. It is recommended to not use [Any] when possible.
 */
public fun <T> sidedDelegate(
    clientValue: () -> T,
    serverValue: () -> T,
): ReadOnlyProperty<Any?, T> = SidedGetterDelegate(clientValue, serverValue)

/**
 * Lazy sided delegate whose values are initialized lazily.
 */
private class LazySidedDelegate<T>(
    clientValue: () -> T,
    serverValue: () -> T,
 ...
 */
private class SidedGetterDelegate<T>(
    private val clientValue: () -> T,
    private val serverValue: () -> T,
) : ReadOnlyProperty<Any?, T> {
    override fun getValue(
        thisRef: Any?,
        property: KProperty<*>,
    ): T = runForDist(clientValue, serverValue)
}
