...
public class KotlinLanguageProvider : IModLanguageProvider {
    ...
    private class KotlinModTarget(
        private val className: String,
    ) : IModLanguageProvider.IModLanguageLoader {
        override fun <T> loadMod(
            info: IModInfo,
            ...
            try {
                val ktContainer =
                    Class.forName(
                        "thedarkcolour.kotlinforforge.KotlinModContainer",
                        true,
                        Thread.currentThread().contextClassLoader,
                    )
                LOGGER.debug(
                    Logging.LOADING,
                    "Loading KotlinModContainer from classloader {} - got {}}",
                    Thread.currentThread().contextClassLoader,
                    ktContainer.classLoader,
                )
                val constructor =
                    ktContainer.getConstructor(
                        IModInfo::class.java,
                        String::class.java,
