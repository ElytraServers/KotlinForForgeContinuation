package thedarkcolour.kotlinforforge

import net.minecraftforge.eventbus.api.IEventBus
import net.minecraftforge.fml.ModLoadingContext

/**
 * Mod loading context for mods made with Kotlin for Forge.
 */
public class KotlinModLoadingContext(
    private val container: KotlinModContainer,
) {
    /** Mods should access through [MOD_BUS] */
    public fun getKEventBus(): IEventBus = container.eventBus

    public companion object {
        /** Mods should access through [MOD_CONTEXT] */
        public fun get(): KotlinModLoadingContext = ModLoadingContext.get().extension()
    }
}
