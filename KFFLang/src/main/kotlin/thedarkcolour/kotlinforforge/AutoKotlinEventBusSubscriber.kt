...
public object AutoKotlinEventBusSubscriber {
     ...
     */
    public fun inject(
        mod: KotlinModContainer,
        scanData: ModFileScanData,
        ...
        val data =
            scanData.annotations.filter { annotationData ->
        ...
        for (annotationData in data) {
            ...
            val busTargetHolder =
                annotationData.annotationData.getOrDefault(
                    "bus",
                    ModAnnotation.EnumHolder(null, "FORGE"),
                ) as ModAnnotation.EnumHolder
            ...
            if (mod.modId == modid && FMLEnvironment.dist in sides) {
                ...
                try {
                    ...
                    if (unsupported.message?.contains("file facades") == false) {
                        ...
                        LOGGER.debug(
                            Logging.LOADING,
                            "Auto-subscribing kotlin file {} to {}",
                            annotationData.annotationType.className,
                            busTarget,
                        )
                ...
                if (ktObject != null) {
                    try {
                        ...
                        LOGGER.fatal(
                            Logging.LOADING,
                            "Failed to load mod class ${annotationData.annotationType} for @EventBusSubscriber annotation",
                            e,
                        )
    ...
    private fun registerTo(
        any: Any,
        target: Mod.EventBusSubscriber.Bus,
        mod: KotlinModContainer,
    ) {
