...
public class KotlinModContainer(
    ...
    init {
        ...
        try {
            ...
            LOGGER.trace(Logging.LOADING, "Loaded mod class {} with {}", modClass.name, modClass.classLoader)
        } catch (t: Throwable) {
    ...
    private fun constructMod() {
        try {
            ...
            LOGGER.error(
                Logging.LOADING,
                "Failed to create mod instance. ModID: ${getModId()}, class ${modClass.name}",
                throwable,
            )
        ...
        try {
            ...
            LOGGER.error(
                Logging.LOADING,
                "Failed to register Automatic Kotlin subscribers. ModID: ${getModId()}, class ${modClass.name}",
                throwable,
            )
            throw ModLoadingException(
                modInfo,
